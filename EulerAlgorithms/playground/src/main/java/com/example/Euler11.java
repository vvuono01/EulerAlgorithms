...
public class Euler11 {
    ...
    private static long greatestAdjacentProduct(int[][] grid, int numAdajacentNumbers) {
        int[][] verticalProducts = grid;
        int[][] horizontalProducts = grid;
        for (int i = 0; i < 3; i++) {
            verticalProducts = calculateVerticalProducts(verticalProducts);
            horizontalProducts = calculateHorizontalProducts(horizontalProducts);
        }

        long greatestProduct = 0;
        for (int i = 0; i < verticalProducts.length; i++) {
            for (int j = 0; j < verticalProducts[i].length; i++) {
                if (greatestProduct < verticalProducts[i][j]) {
                    greatestProduct = verticalProducts[i][j];
                }
            }
        }
        for (int i = 0; i < horizontalProducts.length; i++) {
            for (int j = 0; j < horizontalProducts[i].length; i++) {
                if (greatestProduct < horizontalProducts[i][j]) {
                    greatestProduct = horizontalProducts[i][j];
                }
            }
        }
    ...
    private static int[][] calculateVerticalProducts(int grid[][]) {
        int[][] verticalProducts = new int[grid.length / 2][grid.length];
        for (int i = 0; i < grid.length - 1; i += 2) {
            verticalProducts[i/2] = new int[grid.length];
            for (int j = 0; j < grid.length; j++) {
                verticalProducts[i][j] = grid[i][j] * grid[i+1][j];
            }
        }
    ...
    private static int[][] calculateHorizontalProducts(int grid[][]) {
        int[][] horizontalProducts = new int[grid.length][grid.length / 2];
        for (int i = 0; i < grid.length; i++) {
            horizontalProducts[i] = new int[grid.length / 2];
            for (int j = 0; j < grid.length - 1; j += 2) {
                horizontalProducts[i][j] = grid[i][j] * grid[i][j+1];
            }
        }
